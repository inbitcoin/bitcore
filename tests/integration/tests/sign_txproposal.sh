...
test_header="testing POST /v1/txproposals/:id/signatures/ ... "
echo -n "${test_header}"

report_basename="signtxproposal"
gen_report_dir > /dev/null

entrypoint="${UTILS_DIR}/sign_txproposal.sh"
loop_msg="signing txproposal of wallet n. %s"
cmd_base="${DOCKER_RUN_D} --name wlt_%s ${IMG_NAME} ${entrypoint} ${report_dir} "${txp}""
