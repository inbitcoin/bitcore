...
test_header="testing PUT /v1/txnotes/:txid/ ... "
echo -n "${test_header}"

report_basename="puttxnote"
gen_report_dir > /dev/null

entrypoint="${UTILS_DIR}/add_txnote.sh"
loop_msg="requesting change of note for a tx of wallet n. %s"
