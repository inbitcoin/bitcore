...
test_header="testing GET /v1/notifications/ ... "
echo -n "${test_header}"
...
if [ -f "${wlt_file}" ]; then
    ...
    for param in "${txp_accepted_data[@]}"; do
		[ "$(jq -r "map(select(.type==\"TxProposalAcceptedBy\")) | map(.data) | all(has(\"${param}\"))" ${wlt_file} )" == "true" ] || \
			handle_error
	done
    ...
    for param in "${txp_finally_accepted_data[@]}"; do
		[ "$(jq -r "map(select(.type==\"TxProposalFinallyAccepted\")) | map(.data) | all(has(\"${param}\"))" ${wlt_file} )" == "true" ] || \
			handle_error
	done
    declare -a incoming_tx_data
	incoming_tx_data=( txid address amount tokenAddress )
    for param in "${incoming_tx_data[@]}"; do
		[ "$(jq -r "map(select(.type==\"NewIncomingTx\")) | map(.data) | all(has(\"${param}\"))" ${wlt_file} )" == "true" ] || \
			handle_error
	done
    declare -a outgoing_tx_data
	outgoing_tx_data=( txProposalId creatorId amount message txid )
    for param in "${outgoing_tx_data[@]}"; do
		[ "$(jq -r "map(select(.type==\"NewOutgoingTx\")) | map(.data) | all(has(\"${param}\"))" ${wlt_file} )" == "true" ] || \
			handle_error
	done
